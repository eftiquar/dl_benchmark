...
parser.add_argument('--lr-steps', default='25,50,75', type=str,
...
def main():
    if opt.mode == 'symbolic':
        ...
        if opt.dataset == 'imagenet':
            lr_steps = [int(x) for x in opt.lr_steps.split(',') if x.strip()]
            epoch_size = max(int(1281167 / (opt.batch_size * kv.num_workers * gpus)), 1)
            optimizer_params['lr_scheduler'] = multi_factor_scheduler(0, epoch_size, step=lr_steps, factor=0.1)
        ...
        mod.fit(train_data,
                ...
                # epoch_end_callback = mx.callback.do_checkpoint(os.path.join(opt.prefix,'image-classifier-%s'% opt.model)),
                optimizer = 'sgd',
                optimizer_params = optimizer_params,
                initializer = mx.init.Xavier(magnitude=2))
        mod.save_params(os.path.join(opt.prefix, 'image-classifier-%s-%d-final.params'%(opt.model, opt.epochs)))
    else:
        if opt.mode == 'hybrid':
            net.hybridize()
